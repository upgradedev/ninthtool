...
then settled it: both told one page from another, and two of the pages declare a required argument
and answer identically when it is left out, which is not visible without calling the tool twice. The protocol was written and committed before any page ran, the failure
