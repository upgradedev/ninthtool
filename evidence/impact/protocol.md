...
### 2026-09-02, later still. The second wave ran, and both rows told pages apart.
...
**P5 returned 2 distinct verdicts across the four pages, and P6 returned 2.** Both rows discriminate.
So the flat execution half in wave 1 was this study's own read only default, exactly as the previous
amendment reasoned, and not a limit on what the instrument reaches.

Two settled findings on independently authored code, neither reachable from a tool list:
`recommend_provider` on one page and `getAvailability` on another each declare a required property
and answer identically when it is omitted. You cannot see that without calling the tool twice.

**What this does not license.** It does not overturn the preregistered failure, it cannot, and it is
not evidence that any maintainer considers those findings worth fixing. No page is named as
defective in any judge-facing document, and no maintainer was contacted.

**The honest limit on the safety story.** `readOnlyHint` is the page's own claim, and this suite
exists to doubt exactly that class of claim. What is enforced is narrower and it is enforced in code
at `src/probe/observe.js:903` and `:1010` by a strict `=== true`: nothing outside a page's own
declared read only set is ever called. `launchWorldMonitor` was excluded by that filter and the
exclusion is visible in the run record. Measured once alongside the run: no file reached the
operator's Downloads folder, 121 entries before and after.
